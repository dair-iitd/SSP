...
def main():
    ...
    neutral_ind_pred = [i for i in range(len(preds)) if preds[i] == "neutral"]
    ent_ind_pred = [i for i in range(len(preds)) if preds[i] == "entailment"]
    con_ind_pred = [i for i in range(len(preds)) if preds[i] == "contradiction"]

    #neutral_pred = [preds[it] for it in neutral_ind]
    #neutral_acc = sum(1 for a in neutral_pred if a == "neutral")/len(neutral_pred)
    gold_ = [gold[it] for it in neutral_ind_pred]
    try:
        neutral_prec = sum(1 for a in gold_ if a == "neutral")/len(neutral_ind_pred)
    except:
        neutral_prec = 0
    ...
    gold_ = [gold[it] for it in ent_ind_pred]
    ent_prec = sum(1 for a in gold_ if a == "entailment")/len(ent_ind_pred)

    con_pred = [preds[it] for it in con_ind]
    con_acc = sum(1 for a in con_pred if a == "contradiction")/len(con_pred)
    gold_ = [gold[it] for it in con_ind_pred]
    con_prec = sum(1 for a in gold_ if a == "contradiction")/len(con_ind_pred)
    ...
    print("Precision: ", acc)
    print("Neutral precision: ", neutral_prec)
    print("Contradiction precision: ", con_prec)
    print("Entailment precision: ", ent_prec)
