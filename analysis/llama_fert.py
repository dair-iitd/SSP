...
def get_response_from_llama(example, model):
    ...
    response, word_probs = parse_tsv_example(task, example, completion, word_probs)

    model.cleanup()
    return response, word_probs, completion
...
def main():
    ...
    for i, example in enumerate(bar):
        ...
        #if i < 90:
        #    continue
        fert = get_response_from_llama(example, model)
        ferts.append(fert)
        #pdb.set_trace()
