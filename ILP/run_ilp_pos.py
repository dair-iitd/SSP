...
if __name__=='__main__':
    ...
    LANG = args.lang
    similarity = np.load("/home/vipul/llm/hpc_emnlp/data/new/{}/tgt_sim.npy".format(LANG))
    K = 100
    #TAU = 0.6
    M = args.n_exemplars
    if args.it == 8:
        pdb.set_trace()
        M=1
    LABELS = ['ADJ', 'ADP', 'ADV', 'AUX', 'CCONJ', 'DET', 'INTJ', 'NOUN', 'NUM', 'PART', 'PRON', 'PROPN', 'PUNCT', 'SCONJ', 'VERB', 'SYM', 'X']
    BETAS = [0.1]*(len(LABELS)-2) + [0,0]
    ...
    if args.zplus:
        ...
        if args.rem_lab_cons:
            ...
            if "llama" in args.data_dir:
                ...
                np.save(LANG + "_80percile_"+str(M)+"ex_ILP_seq_grb_zgul_lam_"+str(args.lamda)+"_"+str(args.pool_size)+"_v5.npy", selections)
